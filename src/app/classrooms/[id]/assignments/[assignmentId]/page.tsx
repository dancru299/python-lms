    ...
    <div className="min-h-screen bg-gray-200">
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center gap-4">
          <Link href={`/classrooms/${classroomId}`} className="text-gray-600 hover:text-gray-900">
            <i className="fa-solid fa-arrow-left"></i>
          </Link>
          <div>
            <h1 className="text-xl font-bold text-gray-900">{assignment.title}</h1>
            <p className="text-sm text-gray-500">
              {assignment.type === "test" ? "Bài ki?m tra" : "Bài t?p v? nhà"}
      ...
      <main className="max-w-[1400px] mx-auto px-4 py-6 grid lg:grid-cols-2 gap-4">
        <section className="bg-white rounded-lg border border-gray-300 p-5">
          <h2 className="text-3xl font-semibold text-gray-900 mb-4 border-b-2 border-orange-500 pb-2">Ð? Bài</h2>
          <div className="min-h-[70vh] text-gray-800 prose max-w-none">
            {assignment.questionHtml ? (
              <NoCopyHtml html={assignment.questionHtml} />
            ) : (
              <p>Chua có n?i dung d? bài.</p>
            )}
          </div>
        </section>

        <section className="bg-white rounded-lg border border-gray-300 p-5">
          <h2 className="text-3xl font-semibold text-gray-900 mb-4 border-b-2 border-orange-500 pb-2">N?p Bài (Code Editor)</h2>

          <div className="mb-4">
            <label className="block text-2xl font-semibold text-gray-900 mb-2">H? và Tên Sinh Viên:</label>
            <input className="input text-lg" value={profileUser?.name || session.name} readOnly />
          </div>

          <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200 text-sm text-gray-700">
            <div><strong>L?p:</strong> {profileUser?.profile?.gradeLevel || "Chua c?p nh?t"}</div>
            <div><strong>Tru?ng:</strong> {profileUser?.profile?.school || "Chua c?p nh?t"}</div>
            <div><strong>S? di?n tho?i:</strong> {profileUser?.profile?.phone || "Không có"}</div>
          </div>
